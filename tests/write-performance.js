// performance tests are not really appropriate across different environments
...
const createTestModel = (dbPath) =>
  modelSetup({
    dbName: dbPath,
    tables: (db) => {
      db.exec(
        "CREATE TABLE IF NOT EXISTS testEvents (id INTEGER PRIMARY KEY, data TEXT)",
      );
    },
    queries: (db) => ({
      logEvent: db.prepare(
        "INSERT INTO testEvents (data) VALUES (?) RETURNING *",
      ),
    }),
    methods: (queries) => ({
