...
const initQueue = function (options = {}) {
  ...
  const create = db.query(
    "CREATE TABLE IF NOT EXISTS queue ( id INTEGER PRIMARY KEY AUTOINCREMENT, datetime INTEGER NOT NULL, user TEXT, ip TEXT, cmd TEXT NOT NULL, data TEXT); ",
  );
  create.run();
  ...
  const queries = {
    create,
    cycle: db.prepare(
      "SELECT id, datetime, user, ip, cmd, data FROM queue WHERE id >= $start ORDER BY id LIMIT 1000 OFFSET $offset",
    ),
    cycleStop: db.prepare(
      "SELECT id, datetime, user, ip, cmd, data FROM queue WHERE id >= $start AND id < $stop ORDER BY id LIMIT 1000 OFFSET $offset",
    ),
    getRowByID: db.prepare(
      "SELECT id, datetime, user, ip, cmd, data FROM queue WHERE id = $id",
    ),
    storeRow: db.prepare(
      "INSERT INTO queue (datetime, user, ip, cmd, data) VALUES($datetime,$user,$ip,$cmd,$data) RETURNING *",
    ),
    getLastRow: db.prepare(
      "SELECT id, datetime, user, ip, cmd, data FROM queue ORDER BY id DESC LIMIT 1",
    ),
  };
